...
# check test performance framework is available in tcl-core:
apply {{} {
  foreach lib [list \
    [file join [info library] tests-perf test-performance.tcl] \
    [file join [file dirname [info library]] tests-perf test-performance.tcl] \
  ] {
    if {[file exists $lib]} {
      source $lib
      return -code return {}; # found/loaded - use it
    }
  }
}}

# last known version of the framework:
